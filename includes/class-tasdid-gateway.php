...
class Tasdid_Gateway
{
    ...
    {
        if (defined('TASDID_GATEWAY_VERSION')) {
            $this->version = TASDID_GATEWAY_VERSION;
        } else {
            $this->version = '1.0.0';
        }
        $this->plugin_name = 'tasdid-gateway';
    ...
    private function load_dependencies()
    {
        ...
        /**
         * The class responsible adding for Tasdid callback for wordpress
         */
        require_once plugin_dir_path(dirname(__FILE__)) . 'includes/class-tasdid-gateway-callback.php';
     ...
     *
     * Uses the Tasdid_Gateway_i18n class in order to set the domain and to register the hook
     * with WordPress.
     *
     * @since    1.0.0
     * @access   private
    ...
    {
        $plugin_filters = new Tasdid_Gateway_Filters();
        $this->loader->add_filter('woocommerce_order_data_store_cpt_get_orders_query', $plugin_filters, 'add_tasdid_meta_to_query', 10, 2);
    ...
    {
        $plugin_filters = new Tasdid_Gateway_Callback();
     ...
     *
     * @return    Tasdid_Gateway_Loader    Orchestrates the hooks of the plugin.
     * @since     1.0.0
     */
